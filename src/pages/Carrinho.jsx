      ...
      {cart.length === 0 ? (
        ...
        <div className="cart-content-grid">
          <div className="cart-items-list">
            {/* O AnimatePresence permite animar itens que saem do DOM */}
            <AnimatePresence mode='popLayout'>
              {cart.map(item => (
                <motion.div 
                  key={item.id} 
                  className="cart-item-card"
                  layout // Faz os itens vizinhos deslizarem suavemente
          ...
          {/* O resumo também ganha uma entrada suave */}
          <motion.div 
            className="cart-summary-card"
            initial={{ opacity: 0, opacity: 0, x: 20 }}
            animate={{ opacity: 1, x: 0 }}
